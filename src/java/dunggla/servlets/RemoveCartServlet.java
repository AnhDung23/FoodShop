        ...
        try {
            ...
            if (session != null) {
                ...
                if (cart != null) {
                    // Get item remove
                    String[] items;
                    items = request.getParameterValues("cbxItem");

                    if (items != null) {
                        String flagConfirm = request.getParameter("txtFlagConfirm");
                        // Send message confirm delete
                        if (flagConfirm == null) {
                            request.setAttribute("MSG_REMOVE", "Check box to confirm remove");
                            request.setAttribute("CBX_ACTION", items);

                        } else if (flagConfirm.equals("flagConfirm")) {
                            // Confirm
